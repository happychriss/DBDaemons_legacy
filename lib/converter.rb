...
class Converter
  ...
  def run_conversion(data_jpg, mime_type, source, page_id,convert_as_foto)
    ...
    begin
      t = Thread.new do
        ...
        begin
          convert_data(data_jpg, mime_type, source, page_id,convert_as_foto)
        rescue => e
  ...
  def convert_data(data_jpg, mime_type, source, page_id,convert_as_foto)
    ...
    begin
      ...
      if [:PDF].include?(mime_type)
        ...
        unless convert_as_foto
          puts "------------ Process as normal page with contrast enhancement ----------"
          exec("convert '#{fopath}.orient' -auto-level -normalize -brightness-contrast 30x75 '#{fopath}.orient'")
        end

        exec("convert '#{fopath}.orient' -resize 2480x3507 -background white -gravity center -extent 2480x3507 -density 300  #{fopath}.ppm")
        exec("unpaper -v --overwrite  --post-size a4 --sheet-size a4 --no-grayfilter --no-blackfilter  --pre-border 0,120,0,0 '#{fopath}.ppm' '#{fopath}.unpaper'")
        ...
        if @ocr_tesseract_available or @ocr_abby_available
          ...
          if source == PAGE_SOURCE_UPLOADED
            ...
            converter_upload_pdf(result_txt, File.open(fpath), File.open(fopath+ '.pdf'), page_id)
          else
            puts "Return normal Converted PDF as orginal"
            converter_upload_pdf(result_txt, File.open(fopath+ '.pdf'),"", page_id)
          end
        end
        ...
        converter_upload_pdf(result_txt, File.open(fpath), File.open(pdf_path), page_id)
  ...
  def converter_upload_jpgs(result_jpg, result_sjpg, page_id)
    ...
    RestClient.post @web_server_uri + '/convert_upload_jpgs', {:page => {:result_sjpg => result_sjpg, :result_jpg => result_jpg, :id => page_id}}
  end

  def converter_upload_pdf(text, org_data, pdf_data, page_id)
